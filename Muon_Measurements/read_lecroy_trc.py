# -*- coding: utf-8 -*-
"""
Created on Thu Nov 17 04:50:59 2016
...
def trace_info(m):
    mem = m['WAVE_ARRAY_COUNT']
    nseq = m['SUBARRAY_COUNT']
    tstep = round(m['HORIZ_INTERVAL'] * 1e9, 4) #ns
    tb = m['TIMEBASE'].split('_')
    t = int(tb[0]) * 10
    tstr = tb[1].split('/')[0]
    tseq = {
      's': lambda t: t,
    ...
    lseq = int(mem / nseq)
    return nseq, lseq, tstep, tseq


# print('Reading file....')
# path = "data/dc/15_03_24/"
# filename = 'C2--xtlk--00000.trc'
# print(path+filename)
# x, y, d = readTrc(path+filename)
